        ...
        <main className="min-h-screen bg-transparent transition-colors duration-300">
            ...
            <div className="max-w-4xl mx-auto p-6 pt-10">
                <div className="flex items-center justify-between mb-8">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h1 className="text-2xl font-bold text-white dark:text-white">Parking Lot</h1>
                            <p className="text-white/60 dark:text-gray-400">Manage questions from your students.</p>
                        </div>
                    </div>
                    <Button
                        variant="secondary"
                ...
                <div className="space-y-8">
                    ...
                    <section>
                        ...
                        {unanswered.length === 0 ? (
                            <div className="bg-white/10 backdrop-blur-sm rounded-xl p-8 text-center border border-white/10 dark:border-slate-700">
                                <p className="text-gray-400">No new questions. All clear! 🎉</p>
                            </div>
                        ) : (
                            <div className="grid gap-4">
                                {unanswered.map((q) => (
                                    <div key={q.id} className="bg-white/10 backdrop-blur-sm rounded-xl p-6 shadow-sm border-l-4 border-red-500 animate-slide-in">
                                        <div className="flex justify-between items-start gap-4">
                                            <div className="flex-1">
                                                <p className="text-lg font-medium text-white dark:text-white mb-2">{q.question}</p>
                                                <div className="flex items-center gap-2 text-sm text-white/60 dark:text-gray-400">
                                            ...
                                            <div className="flex gap-2">
                                                ...
                                                <button
                                                    ...
                                                    className="p-2 bg-gray-100 dark:bg-slate-700 text-white/60 dark:text-gray-400 rounded-lg hover:bg-red-100 dark:hover:bg-red-900/30 hover:text-red-500 transition-colors"
                                                    title="Delete"
                                                >
                    ...
                    {answered.length > 0 && (
                        <section>
                            <h2 className="text-lg font-bold text-white/60 dark:text-gray-400 mb-4">
                                Answered ({answered.length})
                            </h2>
                            <div className="grid gap-4 opacity-75">
                                {answered.map((q) => (
                                    <div key={q.id} className="bg-white/5 dark:bg-slate-800/50 rounded-xl p-6 border border-white/20 dark:border-slate-700">
                                        <div className="flex justify-between items-start gap-4">
                                            <div className="flex-1">
            ...
            {answeringId && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                    ...
                    <div className="relative bg-white/10 backdrop-blur-sm rounded-2xl p-6 w-full max-w-md shadow-2xl animate-bounce-in">
                        <h3 className="text-xl font-bold text-white dark:text-white mb-4">Answer Question</h3>
                        <form onSubmit={handleAnswerSubmit}>
                            <textarea
                                value={answerText}
                                onChange={(e) => setAnswerText(e.target.value)}
                                placeholder="Type your answer here..."
                                className="w-full h-32 p-4 rounded-xl border border-white/20 dark:border-slate-600 bg-transparent text-white dark:text-white focus:ring-2 focus:ring-blue-500 outline-none resize-none mb-4"
                                autoFocus
                            />
